function varargout = GUI_PWV_coronal_auto_choice(varargin)
% GUI_PWV_CORONAL MATLAB code for GUI_PWV_auto_coronal.fig
%      GUI_PWV_CORONAL_AUTO_CHOICE, by itself, creates a new GUI_PWV_CORONAL or raises the existing
%      singleton*.
%
%      H = GUI_PWV_CORONAL_AUTO_CHOICE returns the handle to a new GUI_PWV_CORONAL or the handle to
%      the existing singleton*.
%
%      GUI_PWV_CORONAL_AUTO_CHOICE('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in GUI_PWV_CORONAL_AUTO_CHOICE.M with the given input arguments.
%
%      GUI_PWV_CORONAL_AUTO_CHOICE('Property','Value',...) creates a new GUI_PWV_CORONAL_AUTO_CHOICE or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before GUI_PWV_coronal_auto_choiceOpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to GUI_PWV_coronal_auto_choice_OpeningFcn via varargin.
%
...
% Last Modified by GUIDE v2.5 27-Sep-2022 17:56:31

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @GUI_PWV_coronal_auto_choice_OpeningFcn, ...
                   'gui_OutputFcn',  @GUI_PWV_coronal_auto_choice_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before GUI_PWV_coronal_auto_choice is made visible.
function GUI_PWV_coronal_auto_choice_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to GUI_PWV_coronal (see VARARGIN)

% Choose default command line output for GUI_PWV_coronal_auto_choice
...
global transit_time_asc_desc transit_time_desc_dia transit_time_asc_dia info info_global C_total...
    ...
    ObjectPolarity_mode flag_diapgragm z_ref z_ref_diaphragm X info_axial flag_mode
...
%      'FaceColor','texturemap');
    ...
    %- Subplot
    figure(1);subplot(row,col,k_plot);hold all; imshow(Im_axial{k_ref_asc});str = ['z=',int2str(z(k_ref_asc)),'mm'];title(str,'Color','r');
    figure(1),viscircles(centers_desc(k_ref_asc,:), radii_desc(k_ref_asc),'Color','b');figure(1),viscircles(centers_asc(k_ref_asc,:), radii_asc(k_ref_asc),'Color','r');
    %- 3D plot
    %- Descending aorta
...
%     axes(handles.axes1),plot3(C_desc_3D(k_ref_asc,1),C_desc_3D(k_ref_asc,2),C_desc_3D(k_ref_asc,3),'*k'); 
    %- Ascending aorta
...
%     axes(handles.axes1),plot3(C_asc_3D(k_ref_asc,1),C_asc_3D(k_ref_asc,2),C_asc_3D(k_ref_asc,3),'*k'); 
end

%- Find ascending and descending aorta for slices below reference slice
flag_asc = 1; 
if flag_diapgragm==1
    for k=k_ref_asc-1:-1:k_ref_dia-1
        I = Im_axial{k};
        k_plot = k_plot-1;
        figure(1);subplot(row,col,k_plot);hold all; imshow(I);str = ['z=',int2str(z(k)),'mm'];title(str);
        
        %- Select potential descending and ascending aorta
        flag_plot = 0;
%         [centers_desc_list radii_desc_list centers_asc_list radii_asc_list] = select_potential_desc_asc_dark_mode(I,centers_desc_ref,radii_desc_ref,centers_asc_ref,radii_asc_ref,flag_asc)
        [centers_desc_slice radii_desc_slice centers_asc_slice radii_asc_slice] = select_potential_desc_asc_dark_mode(I,centers_desc_ref,radii_desc_ref,centers_asc_ref,radii_asc_ref,flag_asc)
        
        %- Descending aorta
        centers_desc(k,:) = centers_desc_slice;
        radii_desc(k) = radii_desc_slice;
        centers_desc_ref = centers_desc_slice;
        radii_desc_ref = radii_desc_slice;
        figure(1),viscircles(centers_desc(k,:), radii_desc(k),'Color','b');
        C_desc_3D(k,:) = [centers_desc(k,:) z(k)];
        R_desc_3D(k) = radii_desc(k);
        %- Define descending aorta for 3D plot
        x_desc = C_desc_3D(k,1)+R_desc_3D(k)*cos(teta);
        y_desc = C_desc_3D(k,2)+R_desc_3D(k)*sin(teta) ;
        z_desc = C_desc_3D(k,3)+zeros(size(x_desc)) ;
        axes(handles.axes1),patch(x_desc,y_desc,z_desc,'b');
%         axes(handles.axes1),plot3(C_desc_3D(k,1),C_desc_3D(k,2),C_desc_3D(k,3),'*k'); 
                
        %- Ascending aorta
        if flag_asc == 1
            center_asc_diff = distance_from_reference(centers_asc_slice,centers_asc_ref)
            radius_asc_diff = abs(radii_asc_slice - radii_asc_ref);
            if center_asc_diff<10 && radius_asc_diff<5
                centers_asc(k,:) = centers_asc_slice;
                radii_asc(k) = radii_asc_slice;
                centers_desc(k,:) = centers_desc_slice;
                radii_desc(k) = radii_desc_slice;
                centers_desc_ref = centers_desc_slice;
                radii_desc_ref = radii_desc_slice;
                figure(1),viscircles(centers_asc(k,:), radii_asc(k),'Color','r');
                C_asc_3D(k,:) = [centers_asc(k,:) z(k)];
                R_asc_3D(k) = radii_asc(k);
                %- Define ascending aorta
                x_asc = C_asc_3D(k,1)+R_asc_3D(k)*cos(teta);
                y_asc = C_asc_3D(k,2)+R_asc_3D(k)*sin(teta) ;
                z_asc = C_asc_3D(k,3)+zeros(size(x_asc));
                axes(handles.axes1),patch(x_asc,y_asc,z_asc,'r');
    %            axes(handles.axes1),plot3(C_asc_3D(k,1),C_asc_3D(k,2),C_asc_3D(k,3),'*k');
            else
                 centers_asc(k,:) = [0 0];
                 radii_asc(k) = 0;
                 flag_asc = 0;
        ...
        clear centers_desc_list centers_desc_distance radii_desc_list centers_asc_list centers_asc_distance radii_asc_list ind_small_enough ind_large_enough ind
               
    end
end

%- Find ascending and descending aorta for slices above reference slice
k_plot = k_ref_asc; flag_asc = 1;
centers_desc_ref = centers_desc(k_ref_asc,:); radii_desc_ref = radii_desc(k_ref_asc); centers_asc_ref = centers_asc(k_ref_asc,:); radii_asc_ref = radii_asc(k_ref_asc);
for k= k_ref_asc+1:1:size(info,2)
        I = Im_axial{k};
        k_plot = k_plot+1;
        figure(1);subplot(row,col,k_plot);hold all; imshow(I);str = ['z=',int2str(z(k)),'mm'];title(str);
...
%         [centers_desc_list radii_desc_list centers_asc_list radii_asc_list] = select_potential_desc_asc_dark_mode(I,centers_desc_ref,radii_desc_ref,centers_asc_ref,radii_asc_ref,flag_asc)
        [centers_desc_slice radii_desc_slice centers_asc_slice radii_asc_slice] = select_potential_desc_asc_dark_mode(I,centers_desc_ref,radii_desc_ref,centers_asc_ref,radii_asc_ref,flag_asc)
        if isempty(centers_desc_slice) || isempty(centers_asc_slice) || isequal(centers_desc_slice,centers_desc(k-1,:)) || isequal(centers_asc_slice,centers_asc(k-1,:))
                break
        else
            %- Descending aorta
            centers_desc(k,:) = centers_desc_slice;
            radii_desc(k) = radii_desc_slice;
            centers_desc_ref = centers_desc(k,:);
            %- Ascending aorta
            centers_asc(k,:) = centers_asc_slice;
            radii_asc(k) = radii_asc_slice;
            centers_asc_ref = centers_asc(k,:);
        end
        
        %- Ascending aorta
        if isempty(centers_desc_slice)
                break
        else
            centers_desc(k,:) = centers_desc_slice;
            radii_desc(k) = radii_desc_slice;
        ...
        %- PLOTS
        if centers_asc(k,2)>(centers_asc(k-1,2)-1.5) && centers_desc(k,2)>(centers_desc(k-1,2)-5)
            %- Descending aorta
            figure(1),viscircles(centers_desc(k,:), radii_desc(k),'Color','b');
            if centers_desc(k,1)~=0
                C_desc_3D(k,:) = [centers_desc(k,:) z(k)];
                R_desc_3D(k) = radii_desc(k);
                x_desc = C_desc_3D(k,1)+R_desc_3D(k)*cos(teta);
                y_desc = C_desc_3D(k,2)+R_desc_3D(k)*sin(teta) ;
...
%             axes(handles.axes1),plot3(C_asc_3D(k,1),C_asc_3D(k,2),C_asc_3D(k,3),'*k');
            ...
            centers_desc(k,:) = [];
            centers_asc(k,:) = [];
            figure(1);subplot(row,col,k_plot);hold all; imshow(Im_axial{k});str = ['z=',int2str(z(k)),'mm'];title(str);
            break
        end
...
%- Measure the top of the aortic arch
GUI_aortic_arch_selection
flag_aortic_arch_mode = getappdata(0,'flag_aortic_arch_mode')
if strcmp(flag_aortic_arch_mode,'interpolation')
    %- Stitch parts together
    ind1 = find(C_asc_3D(:,1)~=0);
    C1x = (C_asc_3D(ind1,1)); C1y =  (C_asc_3D(ind1,2)); C1z = (C_asc_3D(ind1,3));
    C1 = [C1x C1y C1z];
    ind3 = find(C_desc_3D(:,1)~=0);
    C3x = flipud(C_desc_3D(ind3,1)); C3y =  flipud(C_desc_3D(ind3,2)); C3z = flipud(C_desc_3D(ind3,3));
    C3 = [C3x C3y C3z];
    C_total = [C1 ; C3];

    %- Interpolate signal
    C_total = interparc(1000,C_total(:,1),C_total(:,2),C_total(:,3),'spline');
    C_desc_dia = interparc(100,C3(:,1),C3(:,2),C3(:,3),'spline');
    flag_mode = 'Automatic extraction from axial scans with interpolation of the aortic arch'
    
elseif strcmp(flag_aortic_arch_mode,'parasagittal')   
    GUI_PWV_parasagittal_fuse
    %- Request transit times
    ...
    %- Request centreline from the parasagittal panel
    x_centreline_asc_desc = getappdata(0,'x_centreline_asc_desc')
    y_centreline_asc_desc = getappdata(0,'y_centreline_asc_desc')
    z_ref_matrix = getappdata(0,'z_ref_matrix')
    z_ref_diaphragm_matrix = getappdata(0,'z_ref_diaphragm_matrix')
    PixelSpacingpara = getappdata(0,'PixelSpacing')
            
    %- Stitch centrelines from the axial panel
    ind1 = find(C_asc_3D(:,1)~=0);
    C1x = (C_asc_3D(ind1,1)); C1y =  (C_asc_3D(ind1,2)); C1z = (C_asc_3D(ind1,3));
    C1 = [C1x C1y C1z];
    ind3 = find(C_desc_3D(:,1)~=0);
    ...
    %- Convert points from the parasagittal scan in 3D
    C2z = (-y_centreline_asc_desc + y_centreline_asc_desc(1))*PixelSpacingpara/info_global.PixelSpacing(1) + z_ref
    ind_C2 = find(C2z<=C1z(end)); C2z(ind_C2)=[]; 
    C2x = linspace(C1x(end),C3x(1),length(C2z))
    C2y = linspace(C1y(end),C3y(1),length(C2z))
    ...
    C_total = [C1 ; C2 ; C3];
    C_desc_dia = interparc(100,C3(:,1),C3(:,2),C3(:,3),'spline');
    flag_mode = 'Automatic extraction from axial scans with extraction of the aortic arch from parasagittal scans'
end
...
length_asc_desc = calculate_aortic_arch_3D(Xtop*info_axial{1}.PixelSpacing(1),Ytop*info_axial{1}.PixelSpacing(1),Ztop);
if strcmp(flag_aortic_arch_mode,'parasagittal')  
    length_asc_desc = length_asc_desc*info_axial{1}.PixelSpacing(1);
end
...
if flag_diapgragm == 1
    ...
    length_desc_dia = calculate_aortic_arch_3D(Xdown*info_axial{1}.PixelSpacing(1),Ydown*info_axial{1}.PixelSpacing(1),Zdown);
...
% UIWAIT makes GUI_PWV_coronal wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = GUI_PWV_coronal_auto_choice_OutputFcn(hObject, eventdata, handles) 
...
global transit_time_asc_desc transit_time_desc_dia transit_time_asc_dia length_asc_desc length_desc_dia length_asc_dia ...
        ...
        info Dis_Asc Dis_Desc Dis_Dia Tasc Tdesc Tdia Qasc Qdesc Qdia Aasc Adesc Adia info_axial flag_mode
...
if isempty(Tdia)
    ...
    generate_spreadsheet_1_part(file,path,info_axial{1,1},EF1,SV,HR,CO,length_asc_desc,transit_time_asc_desc,PWV_asc_desc,Dis_Asc,Dis_Desc,Tasc,Qasc,Qdesc,Aasc,Adesc,EDV,EF,PP,DBP,SBP,Pes,ENd_est,Ees,tNd,ENd_avg,flag_mode)
else    
    [file,path,indx] = uiputfile('.xls');
    generate_spreadsheet_3_parts(file,path,info_axial{1,1},EF1,SV,HR,CO,length_asc_desc,length_desc_dia,length_asc_dia,transit_time_asc_desc,transit_time_desc_dia,transit_time_asc_dia,PWV_asc_desc,PWV_desc_dia,PWV_asc_dia,Dis_Asc,Dis_Desc,Dis_Dia,Tasc,Qasc,Qdesc,Qdia,Aasc,Adesc,Adia,EDV,EF,PP,DBP,SBP,Pes,ENd_est,Ees,tNd,ENd_avg,flag_mode)
end
cd(path_origin)
...
function [centers_desc radii_desc centers_asc radii_asc] = select_potential_desc_asc_dark_mode(I,centers_desc_ref,radii_desc_ref,centers_asc_ref,radii_asc_ref,flag_asc)
global ObjectPolarity_mode
% while flag_desc==0
centers_desc = []; radii_desc = []; centers_asc = []; radii_asc = [];
for nb_amount = 1:50
        im = imsharpen(I,'Radius',2,'Amount',nb_amount);
        [centers_list, radii_list] = imfindcircles(im,[2 20],'ObjectPolarity',ObjectPolarity_mode);
        if isempty(radii_list)==0
            centers_desc_distance = distance_from_reference(centers_list,centers_desc_ref);
            [c_desc ind_desc] = min(centers_desc_distance);
            if c_desc<10;
                centers_desc = centers_list(ind_desc,:);
                radii_desc = radii_list(ind_desc);
                %- Corresponding Ascending aorta
                if flag_asc == 1
                    centers_asc_distance = distance_from_reference(centers_list,centers_asc_ref);
                    [c_asc ind_asc] = min(centers_asc_distance);
                    centers_asc = centers_list(ind_asc,:);
                    radii_asc = radii_list(ind_asc);
                end
                break
            end
        end
end
if isempty(radii_desc)==1
    nb_amount = 5;
    for nb_radius = 1:20
            im = imsharpen(I,'Radius',nb_radius,'Amount',nb_amount);
            [centers_list, radii_list] = imfindcircles(im,[2 20],'ObjectPolarity',ObjectPolarity_mode);
            if isempty(radii_list)==0
                centers_desc_distance = distance_from_reference(centers_list,centers_desc_ref);
                [c_desc ind_desc] = min(centers_desc_distance);
...
if isempty(radii_desc)==1
    centers_desc = centers_desc_ref;
    radii_desc = radii_desc_ref;
    centers_asc = centers_asc_ref;
    radii_asc = radii_asc_ref;
end
