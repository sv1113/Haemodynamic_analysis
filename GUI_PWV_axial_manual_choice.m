...
global transit_time_asc_desc transit_time_desc_dia transit_time_asc_dia info info_global C_total...
    ...
    ObjectPolarity_mode flag_diapgragm z_ref z_ref_diaphragm info_axial flag_mode
...
centers_desc_ref = centers_desc(k_ref_asc,:); radii_desc_ref = radii_desc(k_ref_asc); centers_asc_ref = centers_asc(k_ref_asc,:); radii_asc_ref = radii_asc(k_ref_asc);
for k= k_ref_asc+1:2:k_ref_asc+50
        Im = Im_axial{k};
        setappdata(0,'im',Im)
        ...
        setappdata(0,'centers_asc_ref',centers_asc_ref)
        setappdata(0,'radii_asc_ref',radii_asc_ref)
        GUI_ref_asc_desc_manual_axial_arch
...
%                 plot3(C_asc_3D(k,1),C_asc_3D(k,2),C_asc_3D(k,3),'*k');
        ...
        elseif flag_quit ==1
            ...
            flag_mode = 'Manual extraction from axial scans with interpolation of the aortic arch'
            break
        elseif flag_quit ==2
            ...
            C_total = [C1 ; C2 ; C3];
            C_desc_dia = interparc(100,C3(:,1),C3(:,2),C3(:,3),'spline');
            flag_mode = 'Manual extraction from axial scans with extraction of the aortic arch from parasagittal scans'
            break
        end
end
...
if flag_diapgragm == 1
    ...
    length_desc_dia = calculate_aortic_arch_3D(Xdown*info_global.PixelSpacing(1),Ydown*info_global.PixelSpacing(1),Zdown);
