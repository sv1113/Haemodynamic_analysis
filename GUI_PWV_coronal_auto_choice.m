...
flag_asc = 1; 
if flag_diapgragm==1
    for k=k_ref_asc+1:4:k_ref_dia+5
        I = Im_axial{k};
        Im_detection = imsharpen(I);
        [centers, radii] = imfindcircles(Im_detection,[3 30],'ObjectPolarity',ObjectPolarity_mode);
%         fig = figure;
%         hold all, imshow(Im_detection), viscircles(centers, radii,'Color','b');
%         close(fig)
        k_plot = k_plot-1;
        ...
        flag_plot = 0;
        [centers_desc_list radii_desc_list centers_asc_list radii_asc_list] = select_potential_desc_asc(I,centers,centers_desc_ref,centers_asc_ref,radii,flag_plot)
        
        %- Descending aorta
        ind_radii_desc_target = find(radii_desc~=0)
        radii_desc_target = radii_desc(ind_radii_desc_target(end));
        [ind_small_enough c] = find(radii_desc_list<radii_desc_target*1.3);
        [ind_large_enough c] = find(radii_desc_list>radii_desc_target*0.7);
        ind = intersect(ind_small_enough,ind_large_enough);
        if isempty(ind)==0
            centers_desc_list = centers_desc_list(ind,:); radii_desc_list = radii_desc_list(ind);
            centers_desc_distance = distance_from_reference(centers_desc_list,centers_desc_ref);
            [c ind_min] = min(centers_desc_distance);
            if c<15
                centers_desc(k,:) = centers_desc_list(ind_min,:);
                radii_desc(k) = radii_desc_list(ind_min);
                centers_desc_ref = centers_desc(k,:);
                radii_desc_ref = radii_desc(k);
            else
                I = Im_axial{k};
                [centers, radii] = imfindcircles(I,[1 100],'ObjectPolarity',ObjectPolarity_mode);
                [centers_desc(k,:) radii_desc(k)] = find_desc_from_ref(I,centers,centers_desc_ref,radii,xmin_desc,xmax_desc,ymin_desc,ymax_desc,xmin_asc,0);
                centers_desc_ref = centers_desc(k,:);
                radii_desc_ref = radii_desc(k);
            end
        else
            centers_desc(k,:) = centers_desc_ref
            radii_desc(k) = radii_desc_ref;
            centers_desc_ref = centers_desc(k,:);
        end
...
%         axes(handles.axes1),plot3(C_desc_3D(k,1),C_desc_3D(k,2),C_desc_3D(k,3),'*k'); 
        ...
        %- Ascending aorta
        ind_centers_asc_target = find(centers_asc(:,1)~=0)
        centers_asc_target = centers_asc(ind_centers_asc_target(end),:);
        if z(k)>-50 && centers_asc_target(1)~=0 && ~isempty(radii_asc_list) && flag_asc == 1;
            centers_asc_distance = distance_from_reference(centers_asc_list,centers_asc_ref);
            radii_asc_diff = abs(radii_asc_list-radii_asc_ref);
            [centers_min ind_centers_min] = min(centers_asc_distance);
            [radii_min ind_radii_min] = min(radii_asc_diff);
            if centers_min<10 && radii_min<1
                centers_asc(k,:) = centers_asc_list(ind_centers_min,:);
                radii_asc(k) = radii_asc_list(ind_centers_min);
...
centers_desc_ref = centers_desc(k_ref_asc,:); radii_desc_ref = radii_desc(k_ref_asc); centers_asc_ref = centers_asc(k_ref_asc,:); radii_asc_ref = radii_asc(k_ref_asc);
for k= k_ref_asc-1:-1:k_ref_asc-10
        I = Im_axial{k};
        ...
        if c<15
            ...
            I = imadjust(X{k},[.2 .4],[],1);
            [centers, radii] = imfindcircles(I,[1 100],'ObjectPolarity',ObjectPolarity_mode);
            [centers_desc_list radii_desc_list centers_asc_list radii_asc_list] = select_potential_desc_asc(X{k},centers,centers_desc_ref,centers_asc_ref,radii,flag_plot);
            if isempty(centers_desc_list)
                break
            else
                centers_desc_distance = distance_from_reference(centers_desc_list,centers_desc_ref);
                [c ind_min] = min(centers_desc_distance);
        ...
        %- Ascending aorta
        if z(k)>-50 && centers_asc(k+1,1)~=0 && isempty(centers_asc_list)==0
            centers_asc_distance = distance_from_reference(centers_asc_list,centers_asc_ref);
            [c ind_min] = min(centers_asc_distance);
            if c<10
                centers_asc(k,:) = centers_asc_list(ind_min,:);
                radii_asc(k) = radii_asc_list(ind_min);
                centers_asc_ref = centers_asc(k,:);
            else
                Ibis = imadjust(I,[.5 .8],[],1);
                [centers_asc_list, radii_asc_list] = imfindcircles(Ibis,[1 100],'ObjectPolarity',ObjectPolarity_mode);
                centers_asc_distance = distance_from_reference(centers_asc_list,centers_asc_ref);
                [c ind_min] = min(centers_asc_distance);
                if c<10
                    centers_asc(k,:) = centers_asc_list(ind_min,:);
...
%         %- PLOTS
%         if centers_asc(k,2)>(centers_asc(k-1,2)-1.5) && centers_desc(k,2)<centers_desc(k-1,2)
...
%             axes(handles.axes1),plot3(C_asc_3D(k,1),C_asc_3D(k,2),C_asc_3D(k,3),'*k');
            ...
            centers_desc(k,:) = [];
            centers_asc(k,:) = [];
            figure(1);subplot(row,col,k_plot);hold all; imshow(X{k});str = ['z=',int2str(z(k)),'mm'];title(str);
            break
        end
...
C1x = flipud(C_asc_3D(C_asc_3D(:,1)~=0,1)); C1y =  flipud(C_asc_3D(C_asc_3D(:,2)~=0,2)); C1z = flipud(C_asc_3D(C_asc_3D(:,3)~=0,3));
C1 = [C1x C1y C1z];
