...
z_loc = PatientPosition(3) - [1:size(Xf,1)].*PixelSpacing;
...
if ~isempty(Tdia)
    ...
    Xf(z_ref_diaphragm_matrix,:,1)=0; Xf(z_ref_diaphragm_matrix,:,2)=0; Xf(z_ref_diaphragm_matrix,:,3)=1;
...
global transit_time_asc_desc transit_time_desc_dia transit_time_asc_dia length_asc_desc length_desc_dia length_asc_dia ...
        ...
        info Dis_Asc Dis_Desc Dis_Dia path_scans path_flow
...
if isempty(Tdia)
    ...
    generate_spreadsheet_1_part(filename,path,info_global,EF1,SV,HR,CO,length_asc_desc,transit_time_asc_desc,PWV_asc_desc,Dis_Asc,Dis_Desc,Tasc,Qasc,Qdesc,Aasc,Adesc,EDV,EF,PP,DBP,SBP,Pes,ENd_est,Ees,tNd,ENd_avg)
else    
    ...
    Length_Ascending_Descending_m = length_asc_desc; Length_Descending_Diaphragm_m = length_desc_dia; Length_Ascending_Diaphragm_m = length_asc_dia;
    ...
    [filename,path,indx] = uiputfile('.xls')
%     generate_spreadsheet_3_parts(filename,path,info,EF1,SV,HR,CO,length_asc_desc,length_desc_dia,length_asc_dia,transit_time_asc_desc,transit_time_desc_dia,transit_time_asc_dia,PWV_asc_desc,PWV_desc_dia,PWV_asc_dia,Dis_Asc,Dis_Desc,Dis_Dia,Tasc,Qasc,Qdesc,Qdia,Aasc,Adesc,Adia)
...
switch length(idx)
    ...
    for k=1:2
        ...
        %- A vector
        id_var = find(strcmp(variables_name,'Area(mm???)'));
    ...
    for k=1:4
        ...
        %- A vector
        clear id_var
        id_var = 7; %find(strcmp(variables_name,'Area(mm?)'));
        eval([variable_output_A{k} '=datacell(Time_ind{k},id_var);'])
        eval(['id_ko = cellfun(@ischar,' variable_output_A{k} ');'])
        eval([variable_output_A{k} '(id_ko)={0};'])
        eval([variable_output_A{k} '= cell2mat(' variable_output_A{k} ');'])
...
function I = delete_black_border(I)
