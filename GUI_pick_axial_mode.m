function varargout = GUI_pick_axial_mode(varargin)
% GUI_PICK_AXIAL_MODE MATLAB code for GUI_pick_axial_mode.fig
%      GUI_PICK_AXIAL_MODE, by itself, creates a new GUI_PICK_AXIAL_MODE or raises the existing
%      singleton*.
%
%      H = GUI_PICK_AXIAL_MODE returns the handle to a new GUI_PICK_AXIAL_MODE or the handle to
%      the existing singleton*.
%
%      GUI_PICK_AXIAL_MODE('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in GUI_PICK_AXIAL_MODE.M with the given input arguments.
%
%      GUI_PICK_AXIAL_MODE('Property','Value',...) creates a new GUI_PICK_AXIAL_MODE or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before GUI_pick_axial_mode_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to GUI_pick_axial_mode_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help GUI_pick_axial_mode
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @GUI_pick_axial_mode_OpeningFcn, ...
                   'gui_OutputFcn',  @GUI_pick_axial_mode_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before GUI_pick_axial_mode is made visible.
function GUI_pick_axial_mode_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to GUI_pick_axial_mode (see VARARGIN)

% Choose default command line output for GUI_pick_axial_mode
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes GUI_pick_axial_mode wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = GUI_pick_axial_mode_OutputFcn(hObject, eventdata, handles) 
...
%% Select Flow files and transit time
    ...
    current_folder = cd;
    dname = uigetdir(path_flow,'Select folder containing axial plane scans');
    cd(dname)
...
function [x y z x0 y0 z0 x_length y_length z_length pix_space Im_axial] = prepare_volume(V,info)
    ...
    x0 = xyz0(1); y0 = xyz0(2);
    x_length = size(V,2); y_length = size(V,3);
    x = x0 + [0:x_length-1]*pix_space(1);
    y = y0 + [0:y_length-1]*pix_space(2); 
    for k=1:size(info,2)
        Im_axial{k} = mat2gray(V(:,:,:,k));
        Im_axial{k} = Im_axial{k};
        z(k) = info{k}.SliceLocation;
    end
    z_length = abs(z(1)-z(end));
    z0 = z(1);
...
if strcmp(ObjectPolarity_mode,'bright')
    %% Descending aorta
...
%     imshow(I),rectangle('Position',[xmin_asc ymin_asc xmax_asc-xmin_asc ymax_asc-ymin_asc]),viscircles(centers_asc_potential,radii_asc_potential,'Color','b')
    ...
    if isempty(radii_desc_ref)==1
        %- Y limits of ROI
       ymin_desc = round(size(I,1)*0.5); ymax_desc = round(size(I,1)*0.8);
        %- X limits of ROI
        xmin_desc = round(size(I,2)*0.4); xmax_desc = round(size(I,2)*0.8);
        %- Find circles within ROI
        centers_desc_potential = centers;radii_desc_potential = radii;
        for k=size(centers_desc_potential,1):-1:1
            if centers_desc_potential(k,1)<xmin_desc || centers_desc_potential(k,1)>xmax_desc || centers_desc_potential(k,2)<ymin_desc || centers_desc_potential(k,2)>ymax_desc
...
%     imshow(I),rectangle('Position',[xmin_desc ymin_desc xmax_desc-xmin_desc ymax_desc-ymin_desc]),viscircles(centers_desc_potential,radii_desc_potential,'Color','b')
    ...
    if isempty(radii_asc_ref)==1
        %- Y limits of ROI
        ymin_asc = round(size(I,1)*0.1); ymax_asc = round(size(I,1)*0.55);
        %- X limits of ROI
        xmin_asc = round(size(I,2)*0.35); xmax_asc = round(size(I,2)*0.65);
        %- Find circles within ROI
        centers_asc_potential = centers;radii_asc_potential = radii;
        for k=size(centers_asc_potential,1):-1:1
            if centers_asc_potential(k,1)<xmin_asc || centers_asc_potential(k,1)>xmax_asc || centers_asc_potential(k,2)<ymin_asc || centers_asc_potential(k,2)>ymax_asc
